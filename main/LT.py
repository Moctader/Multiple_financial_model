...
from feature_engineering import get_technical_indicators, get_cyclical_features, calculate_label
...
indicators_to_add = ['sma', 'ema', 'rsi', 'macd', 'bollinger_bands', 'stoch', 'adx', 'cci', 'obv', 'atr']
df = get_technical_indicators(df, indicators_to_add)
df = get_cyclical_features(df)
df['label'] = calculate_label(df, window_size=3)
df.dropna(inplace=True)
df['timestamp'] = pd.to_datetime(df['timestamp'])
df = df.set_index('timestamp')

# Extract features and target
features = df[["open", "close", "ema", "rsi", "macd", "bollinger_hband", "bollinger_lband", "bollinger_mavg", "stoch", "adx", "cci", "obv", "atr", "hour", "day_of_week", "day_of_month", "month", "quarter", "week_of_year", "is_weekend", "is_business_day"]]
target = df['close']
